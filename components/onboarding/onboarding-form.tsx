  ...
  const slides = [
    ...
    <div key="slide2" className="flex flex-col gap-6 min-h-[300px] animate-fade-in">
      ...
      <div>
        ...
        <Input id="age" type="number" min={13} max={100} value={formData.age} onChange={e => {
          const val = e.target.value;
          setFormData({ ...formData, age: val === '' ? '' : Number(val) });
        }} required className="h-12 text-base border-2 border-gray-200 focus:border-indigo-400 rounded-xl px-4" />
      </div>
    </div>,
    // Slide 3: Location
